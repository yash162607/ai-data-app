...
export default function AnalysisContent() {
  ...
  const shareToTelegram = () => {
    if (!analysis) return;
    const text = `📊 AI Analysis: Total $${analysis.totalSpent}, Avg $${analysis.avgCost}`;
    window.open(`https://t.me/share/url?url=${encodeURIComponent(text)}`, "_blank");
  };

  const shareToEmail = () => {
    if (!analysis) return;
    const subject = encodeURIComponent("My AI Analysis Results");
    const body = encodeURIComponent(`Total Spent: $${analysis.totalSpent}\nAverage Cost: $${analysis.avgCost}\nBiggest Expense: ${analysis.biggestExpense?.category}`);
      ...
      <div className="bg-slate-800 p-6 rounded-lg mb-8">
        ...
        <div className="grid md:grid-cols-3 gap-4">
          <div className="bg-slate-700 p-4 rounded-lg">
            ...
            <ul className="space-y-2">
              {analysis.actionPlan?.weekly?.map((item, i) => (<li key={i} className="text-gray-300">📌 {item}</li>))}
            </ul>
          </div>
          <div className="bg-slate-700 p-4 rounded-lg">
            <h4 className="font-bold text-purple-400 mb-2">This Month</h4>
            <ul className="space-y-2">
              {analysis.actionPlan?.monthly?.map((item, i) => (<li key={i} className="text-gray-300">📌 {item}</li>))}
            </ul>
          </div>
          <div className="bg-slate-700 p-4 rounded-lg">
            <h4 className="font-bold text-yellow-400 mb-2">This Quarter</h4>
            <ul className="space-y-2">
              {analysis.actionPlan?.quarterly?.map((item, i) => (<li key={i} className="text-gray-300">📌 {item}</li>))}
            </ul>
          </div>
        </div>
      </div>
            ...
            <tbody>
              {analysis.transactions?.slice(0, 20).map((t, i) => (
                <tr key={i} className="border-b border-slate-700">
                  ...
                  <td className="p-3"><span className="bg-cyan-500/20 px-2 py-1 rounded text-cyan-400">{t.category}</span></td>
